      ...
      <div class="w-full px-6 sm:w-1/2 xl:w-1/3">
        <div class="flex items-center px-5 py-6 bg-white rounded-md shadow-sm">
          ...
          <div class="mx-5">
            ...
            <div class="text-gray-500">пользователи</div>
          </div>
        </div>
      </div>
      ...
      <div class="w-full px-6 mt-6 sm:w-1/2 xl:w-1/3 sm:mt-0">
        <div class="flex items-center px-5 py-6 bg-white rounded-md shadow-sm">
          ...
          <div class="mx-5">
            ...
            <div class="text-gray-500">Локации</div>
          </div>
        </div>
      </div>
      ...
      <div class="w-full px-6 mt-6 sm:w-1/2 xl:w-1/3 xl:mt-0">
        <div class="flex items-center px-5 py-6 bg-white rounded-md shadow-sm">
          ...
          <div class="mx-5">
            ...
            <div class="text-gray-500">Общее количество участников</div>
          </div>
        </div>
      </div>
    </div>
...
const allUsernames = computed(() => {
  ...
  for (let i = 0; i < props.areas.length; i++) {
    if (!usernames.includes(props.areas[i].fullname)) {
      usernames.push(props.areas[i].fullname);
    }
  }
  return usernames;
});
...
const allAddress = computed(() => {
  return props.areas.map((area) => area.address);
});

const allMembers = computed(() => {
  for (let i = 0; i < props.areas.length; i++) {
    for (let j = 0; j < props.areas[i].members.length; j++) {
      if (props.areas[i].members[j].count === undefined) {
        props.areas[i].members[j].count = 0;
      }
      count.value += props.areas[i].members[j].count;
    }
  }
  return count.value;
});

// return props.areas.forEach((area) => {
//   let count = 0;
//   for (let i = 0; i < area.members.length; i++) {
//     count += area.members[i].count;
//   }
